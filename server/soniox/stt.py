...
try:
    ...
    logger.error(
        "In order to use Soniox, you need to `pip install pipecat-ai[soniox]`."
    )
...
class SonioxSTTService(STTService):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        sample_rate = sample_rate or (
            params.sample_rate if params.sample_rate else None
        )
    ...
    async def _keepalive_task_handler(self):
        ...
        try:
            ...
            await self.push_error(
                ErrorFrame(f"{self} error (_keepalive_task_handler): {e}")
            )
    ...
    async def _finalize_if_no_tokens_task_handler(self):
        """If no new tokens are received for a while and there is active transcription (not yet sent to the client),
        finalize the transcription by sending the finalize message to Soniox.
        """
        if (
            not self._websocket
            or self._websocket.closed
            or self._auto_finalize_delay_ms is None
        ):
            return
        ...
        try:
            while True:
                ...
                if self._last_tokens_received:
                    last_token_age_ms = (
                        time.time() - self._last_tokens_received
                    ) * 1000
